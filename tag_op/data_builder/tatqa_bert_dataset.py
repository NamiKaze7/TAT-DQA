...
def string_tokenizer(string: str, tokenizer, current_index) -> Tuple[List[int], List[str], List[int], List[int], int]:
    if not string:
    ...
    return ids, tokens, number_value, token_index, current_index
...
def table_tokenize(table, tokenizer, mapping, answer_type):
    ...
    for i in range(len(table)):
        for j in range(len(table[i])):
            cell_ids, tokens, number_value, token_index, current_index = \
                string_tokenizer(table[i][j], tokenizer, current_index)
            if not cell_ids:
                continue
            table_ids += cell_ids
            table_number_value += number_value
            table_tokens += tokens
            if table_mapping:
                ...
                table_tags += [0 for _ in range(len(cell_ids))]
            table_index += token_index
    return table_tokens, table_ids, table_tags, table_number_value, table_index
    ...
    for i in range(len(table)):
        for j in range(len(table[i])):
            ...
            if table_mapping:
                ...
                table_tags += [0 for _ in range(len(cell_ids))]
            table_index += token_index
    return table_tokens, table_ids, table_tags, table_number_value, table_index, mapping_content
...
def question_tokenizer(question_text, tokenizer):
    return string_tokenizer(question_text, tokenizer, 0)[0]
...
def _concat(question_ids,
            ...
            max_pieces,):
    in_table_index = table_index.copy()
    ...
    table_index[0, question_length:question_length + table_length] = \
        torch.from_numpy(np.array(in_table_index[:table_length]))
    ...
    return input_ids, attention_mask, paragraph_mask, paragraph_number_value, paragraph_index, \
           table_mask, table_number_value, table_index, tags, input_segments
...
class TagTaTQAReader(object):
    ...
    def _make_instance(self, input_ids, attention_mask, token_type_ids, paragraph_mask, table_mask,
                       paragraph_number_value, table_number_value, paragraph_index, table_index, number_order_label,
                        tags_ground_truth, operator_ground_truth, scale_ground_truth, paragraph_tokens,
                       table_tokens, answer_dict, question_id):
        return {
            ...
            "table_index": np.array(table_index),
    ...
    def _to_instance(self, question: str, table: List[List[str]], paragraphs: List[Dict], answer_from: str,
        ...
        # (derivation:str, answer_type:str, facts:list, answer, mapping:dict):
        operator_class = get_operator_class(derivation, answer_type, facts, answer, answer_mapping, scale)
        ...
        # 首先对问题+table进行标注
        table_tokens, table_ids, table_tags, table_number_value, table_index = \
                            table_tokenize(table, self.tokenizer, answer_mapping, answer_type)
        # 对table进行清理，套用tapas的number_order_label构造方式
        for i in range(len(table)):
            for j in range(len(table[i])):
        ...
        table_mask, table_number_value, table_index, tags, token_type_ids = \
            _concat(question_ids, table_ids, table_tags, table_index, table_number_value,
        ...
        return self._make_instance(input_ids, attention_mask, token_type_ids, paragraph_mask, table_mask,
                    ...
                    paragraph_tokens, table_tokens, answer_dict, question_id)
...
class TagTaTQATestReader(object):
    ...
    def _make_instance(self, input_ids, attention_mask, token_type_ids, paragraph_mask, table_mask,
                        paragraph_number_value, table_number_value, paragraph_index, table_index,
                        tags_ground_truth, paragraph_tokens, table_tokens, answer_dict, question_id,
    ...
    def _to_test_instance(self, question: str, table: List[List[str]], paragraphs: List[Dict], answer_from: str,
        ...
        # 然后对sequence_tag标签进行标注
        # 首先对问题+table进行标注
        table_tokens, table_ids, table_tags, table_number_value, table_index, table_mapping_content = \
        ...
        input_ids, attention_mask, paragraph_mask, paragraph_number_value, paragraph_index, \
        table_mask, table_number_value, table_index, tags, token_type_ids = \
            _test_concat(question_ids, table_ids, table_tags, table_index, table_number_value,
                    paragraph_ids, paragraph_tags, paragraph_index, paragraph_number_value,
                    self.sep_start, self.sep_end, self.question_length_limit,
                    self.passage_length_limit, self.max_pieces)
        ...
        return self._make_instance(input_ids, attention_mask, token_type_ids, paragraph_mask, table_mask,
            ...
            table_tokens, answer_dict, question_id, paragraph_mapping_content, table_mapping_content)
