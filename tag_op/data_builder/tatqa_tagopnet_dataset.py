...
OPERATOR_CLASSES = OPERATOR_CLASSES_
...
def paragraph_tokenize(question, paragraphs, tokenizer, mapping, answer_type):
    ...
    # if answer_type == "span":
    #     split_tags = convert_start_end_tags(split_tags, paragraph_index)
...
def paragraph_test_tokenize(question, paragraphs, tokenizer, mapping, answer_type):
    ...
    # if answer_type == "span":
    #     split_tags = convert_start_end_tags(split_tags, paragraph_index)
    return tokens, paragraph_ids, split_tags, word_piece_mask, number_mask, number_value, paragraph_index, mapping_content

def question_table_tokenizer(table, question, tokenizer, mapping):
    # pre-clean the table
    for i in range(len(table)):
        for j in range(len(table[i])):
            if table[i][j] == '' or table[i][j] == 'N/A' or table[i][j] == 'n/a':
                table[i][j] = "NONE"
    # get answer_coordinates and answer_text
    table_mapping = False
    answer_coordinates = None
    answer_text = None
    if "table" in list(mapping.keys()) and len(mapping["table"]) != 0:
        table_mapping = True
        answer_coordinates = mapping["table"]
        answer_coordinates_copy = answer_coordinates.copy()
        answer_coordinates = [(answer_coordinate[0], answer_coordinate[1]) for answer_coordinate in answer_coordinates_copy]
        answer_text = [str(table[answer_coordinate[0]][answer_coordinate[1]]) for answer_coordinate in answer_coordinates_copy]
        del answer_coordinates_copy

    # change table into DataFrame format
    ...
    if not table_mapping:
        tapas_tokens = tokenizer(table, question, padding='max_length', return_tensors="pt")
        input_ids = tapas_tokens["input_ids"]
        token_type_ids = tapas_tokens["token_type_ids"][:,:,:3]
        attention_mask = tapas_tokens["attention_mask"]
        label_ids = torch.zeros_like(input_ids)
    else:
        tapas_tokens = tokenizer(table, question, answer_coordinates, answer_text, padding='max_length', return_tensors="pt")
        input_ids = tapas_tokens["input_ids"]
        token_type_ids = tapas_tokens["token_type_ids"][:, :,:3]
        attention_mask = tapas_tokens["attention_mask"]
        label_ids = tapas_tokens["label_ids"]
    number_mask, number_value = get_number_mask(table)

    return input_ids, attention_mask, token_type_ids, tapas_tokens["query_ids_len"], label_ids, \
                number_mask, number_value, table

def question_table_test_tokenizer(table, question, tokenizer, mapping):
    mapping_content = []
    # pre-clean the table
    for i in range(len(table)):
        for j in range(len(table[i])):
    ...
    if "table" in list(mapping.keys()) and len(mapping["table"]) != 0:
        ...
        mapping_content = [str(table[answer_coordinate[0]][answer_coordinate[1]]) for answer_coordinate in
                       answer_coordinates_copy]
        answer_text = [str(table[answer_coordinate[0]][answer_coordinate[1]]) for answer_coordinate in
                       answer_coordinates_copy]
        del answer_coordinates_copy
    # change table into DataFrame format
    table = pd.DataFrame(table, dtype=np.str)
    ...
    return input_ids, attention_mask, token_type_ids, tapas_tokens["query_ids_len"], label_ids, \
                number_mask, number_value, table, mapping_content

def get_number_order_labels(paragraphs, table, derivation, operator_class, answer_mapping, question_id):

    if operator_class != OPERATOR_CLASSES["DIVIDE"] and operator_class != OPERATOR_CLASSES["CHANGE_RATIO"] and \
...
def _concat(tapas_ids,
            tapas_attention_mask,
            tapas_tags,
            paragraph_ids,
            paragraph_tags,
            tapas_token_type_ids,
            sep_id,
            paragraph_word_piece_mask,
            paragraph_number_mask,
            paragraph_number_value,
            paragraph_index,
            paragraph_tokens,
            mapping,
            miss_target,):
    in_paragraph_word_piece_mask = paragraph_word_piece_mask
    in_paragraph_number_mask = paragraph_number_mask
    in_paragraph_number_value = paragraph_number_value
    in_paragraph_index = paragraph_index
    in_paragraph_tokens = paragraph_tokens
    paragraph_word_piece_mask = torch.zeros_like(tapas_ids)
    paragraph_number_mask = torch.zeros_like(tapas_ids)
    paragraph_mask = torch.zeros_like(tapas_ids)
    paragraph_index = torch.zeros_like(tapas_ids)
    tapas_ids[0, torch.sum(tapas_ids != 0)] = sep_id
    tapas_tags[0, torch.sum(tapas_tags != 0)] = 0
    space_count = int(torch.sum(tapas_ids == 0))
    concat_start = int(torch.sum(tapas_ids != 0))
    if space_count < len(paragraph_ids):
        paragraph_ids = paragraph_ids[:space_count]
        paragraph_tags = paragraph_tags[:space_count]
        in_paragraph_word_piece_mask = in_paragraph_word_piece_mask[:space_count]
        in_paragraph_number_mask = in_paragraph_number_mask[:space_count]
        in_paragraph_number_value = in_paragraph_number_value[:space_count]
        in_paragraph_index = in_paragraph_index[:space_count]
        in_paragraph_tokens = in_paragraph_tokens[:space_count]
    paragraph_number_value = np.ones((tapas_ids.shape[0], tapas_ids.shape[1])) * np.nan
    tapas_ids[0,concat_start:concat_start+len(paragraph_ids)] = torch.from_numpy(np.array(paragraph_ids))
    tapas_tags[0,concat_start:concat_start+len(paragraph_tags)] = torch.from_numpy(np.array(paragraph_tags))
    # tapas_token_type_ids[:,concat_start:concat_start+len(paragraph_tags),0] = 2
    paragraph_word_piece_mask[0, concat_start:concat_start+len(in_paragraph_word_piece_mask)] = \
        torch.from_numpy(np.array(in_paragraph_word_piece_mask))
    paragraph_number_mask[0, concat_start:concat_start+len(in_paragraph_number_mask)] = \
        torch.from_numpy(np.array(in_paragraph_number_mask))
    paragraph_number_value[0, 1:len(in_paragraph_number_value)+1] = \
        torch.from_numpy(np.array(in_paragraph_number_value))
    paragraph_index[0, concat_start:concat_start+len(in_paragraph_index)] = \
        torch.from_numpy(np.array(in_paragraph_index))
    paragraph_mask[0, concat_start:concat_start+len(paragraph_ids)] = 1
    attention_mask = tapas_ids != 0
    attention_mask = attention_mask.type(type(tapas_attention_mask))
    if "paragraph" in list(mapping.keys()) and len(mapping["paragraph"].keys()) != 0:
        if 1 not in paragraph_tags:
            miss_target += 1
    return tapas_ids, attention_mask, paragraph_mask, paragraph_word_piece_mask, \
            paragraph_number_mask, paragraph_number_value, paragraph_index, tapas_tags, tapas_token_type_ids, miss_target

def _test_concat(tapas_ids,
            tapas_attention_mask,
            tapas_tags,
            paragraph_ids,
            paragraph_tags,
    ...
    attention_mask = attention_mask.type(type(tapas_attention_mask))
    return tapas_ids, attention_mask, paragraph_mask, paragraph_word_piece_mask, \
            paragraph_number_mask, paragraph_number_value, paragraph_index, tapas_tags, tapas_token_type_ids
...
class TagTaTQAReader(object):
    def __init__(self, tokenizer, token_length_limit=512):
    ...
    def _make_instance(self, input_ids, attention_mask, token_type_ids, paragraph_mask, paragraph_word_piece_mask,
            paragraph_number_value, paragraph_number_mask, table_number_value, table_number_mask, paragraph_index,
            number_order_label, tags_ground_truth, operator_ground_truth, scale_ground_truth, paragraph_tokens,
                       table, answer_dict, question_id):
        return {
            "input_ids": np.array(input_ids),
            "attention_mask": np.array(attention_mask),
            "token_type_ids": np.array(token_type_ids),
            "paragraph_mask": np.array(paragraph_mask),
            "paragraph_word_piece_mask": np.array(paragraph_word_piece_mask),
            "paragraph_number_value": np.array(paragraph_number_value),
            "paragraph_number_mask": np.array(paragraph_number_mask),
            "table_number_value": np.array(table_number_value),
            "table_number_mask": np.array(table_number_mask),
    ...
    def _to_instance(self, question: str, table: List[List[str]], paragraphs: List[Dict], answer_from: str,
                     answer_type: str, answer:str, facts:dict, derivation: str, answer_mapping: Dict, scale: str, question_id:str):

        # 首先根据answer_type, derivation定义operator
        operator_class = get_operator_class(derivation, answer_type, facts, answer, answer_mapping, scale, OPERATOR_CLASSES)
        scale_class = SCALE.index(scale)
        if operator_class is None:
            self.skip_count += 1
            return None

        # 然后对sequence_tag标签进行标注
        # 首先对问题+table进行标注
        tapas_ids, tapas_attention_mask, tapas_token_type_ids, _, tapas_tags, table_number_mask, table_number_value, table = \
            question_table_tokenizer(table, question, self.tokenizer, answer_mapping)
        # 然后对文本paragraph进行标注，这里要使用tf-idf进行不同paragraph与question的相似度计算
        ...
        number_order_label = get_number_order_labels(paragraphs, table, derivation, operator_class, answer_mapping, question_id)
        # 计算相似度后，返回的paragraph_ids, paragraph_tags都是按照相似度排序的，只需要不断往ids里面填充就好
        input_ids, attention_mask, paragraph_mask, paragraph_word_piece_mask, \
            paragraph_number_mask, paragraph_number_value, paragraph_index, tags, tapas_token_type_ids, self.miss_target = \
            _concat(tapas_ids, tapas_attention_mask, tapas_tags, paragraph_ids, paragraph_tags, tapas_token_type_ids,
            self.sep, paragraph_word_piece_mask, paragraph_number_mask, paragraph_number_value, paragraph_index, paragraph_tokens,
            answer_mapping, self.miss_target)
        if question_id == "ad4735f92df8280ec993bc505a7e2a0f":
            print(number_order_label, operator_class, derivation)
        answer_dict = {"answer_type": answer_type, "answer": answer, "scale": scale, "answer_from": answer_from}
        return self._make_instance(input_ids, attention_mask, tapas_token_type_ids, paragraph_mask,
            paragraph_word_piece_mask, paragraph_number_value, paragraph_number_mask, table_number_value,
            table_number_mask, paragraph_index, number_order_label, tags, operator_class, scale_class,
            paragraph_tokens, table, answer_dict, question_id)
    ...
    def _read(self, file_path: str):
            ...
            for question_answer in questions:
                try:
                    ...
                    instance = self._to_instance(question, table, paragraphs, answer_from,
                                    answer_type, answer,facts,derivation, answer_mapping, scale, question_answer["uid"])
        ...
        print(self.miss_target)
        return instances

class TagTaTQATestReader(object):
    def __init__(self, tokenizer, token_length_limit=512, sep='[SEP]'):
        self.tokenizer = tokenizer
        self.token_length_limit = token_length_limit
        self.sep = self.tokenizer._convert_token_to_id(sep)
        tokens = self.tokenizer._tokenize("Feb 2 Nov")
    ...
    def _make_instance(self, input_ids, attention_mask, token_type_ids, paragraph_mask, paragraph_word_piece_mask,
            paragraph_number_value, paragraph_number_mask, table_number_value, table_number_mask, paragraph_index,
            tags, paragraph_tokens, table, answer_dict, question_id, paragraph_mapping_content, table_mapping_content):
        return {
            "input_ids": np.array(input_ids),
            "attention_mask": np.array(attention_mask),
            "token_type_ids": np.array(token_type_ids),
    ...
    def _to_test_instance(self, question: str, table: List[List[str]], paragraphs: List[Dict], answer_from: str,
                     answer_type: str, answer:str, answer_mapping, scale: str, question_id:str):

        # 然后对sequence_tag标签进行标注
        # 首先对问题+table进行标注
        tapas_ids, tapas_attention_mask, tapas_token_type_ids, _, tapas_tags, table_number_mask, \
        table_number_value, table, table_mapping_content = \
            question_table_test_tokenizer(table, question, self.tokenizer, answer_mapping)
        # 然后对文本paragraph进行标注，这里要使用tf-idf进行不同paragraph与question的相似度计算
        paragraph_tokens, paragraph_ids, paragraph_tags, paragraph_word_piece_mask, paragraph_number_mask, \
        ...
        # 计算相似度后，返回的paragraph_ids, paragraph_tags都是按照相似度排序的，只需要不断往ids里面填充就好
        input_ids, attention_mask, paragraph_mask, paragraph_word_piece_mask, \
            paragraph_number_mask, paragraph_number_value, paragraph_index, tags, tapas_token_type_ids = \
            _test_concat(tapas_ids, tapas_attention_mask, tapas_tags, paragraph_ids, paragraph_tags, tapas_token_type_ids,
            self.sep, paragraph_word_piece_mask, paragraph_number_mask, paragraph_number_value, paragraph_index, paragraph_tokens)
        # passage_mask = torch.zeros_like(input_ids, dtype=torch.long)
        # passage_mask[:,question_ids_len:int(torch.sum(input_ids != 0))] = 1

        answer_dict = {"answer_type": answer_type, "answer": answer, "scale": scale, "answer_from": answer_from}
        return self._make_instance(input_ids, attention_mask, tapas_token_type_ids, paragraph_mask,
            paragraph_word_piece_mask, paragraph_number_value, paragraph_number_mask, table_number_value,
            table_number_mask, paragraph_index, tags, paragraph_tokens, table, answer_dict, question_id,
    ...
    def _read(self, file_path: str):
        ...
        for one in tqdm(dataset):
            ...
            for question_answer in questions:
                annotations.append(question_answer)
                try:
                    question = question_answer["question"].strip()
                    answer_type = question_answer["answer_type"]
                    answer = question_answer["answer"]
                    ...
                    instance = self._to_test_instance(question, table, paragraphs, answer_from,
                                    answer_type, answer, answer_mapping, scale, question_answer["uid"])
                    if instance is not None:
                        instances.append(instance)
                except RuntimeError:
                    print(question_answer["uid"])
